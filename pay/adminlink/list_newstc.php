...
<div class="ureg">
  ...
  <!--right-->
	...
	<?php 
	$numq=mysql_query("select * from tb_newstc");
	...
	$row=listquery("select * from tb_newstc order by id desc limit ".$num.",".$pageso."");
	foreach ($row as $v){
	?>
	  <tr>
		...
		<td width="52"><a href="up_newstc.php?id=<?=$v[id]?>">修改</a></td>
...
$delid=$_GET['delid'];
if($delid){
mysql_query("delete from tb_newstc where id='$delid'");
 exit("<script>alert('删除成功!');window.location.href='list_newstc.php';</script>");
}
?>
...
$del_id=$_POST['del_id'];
     if($del_id!=""){ 
             ...
             for($i=0;$i<$del_num;$i++){ 
                 mysql_query("Delete from tb_newstc where id='$del_id[$i]'"); 				
             }  
            exit("<script>alert('删除成功!');window.location.href='list_newstc.php';</script>");
      }else{ 
            exit("<script>alert('请选择!');window.location.href='list_newstc.php';</script>");
      }
