...
<div class="ureg">
  ...
  <!--right-->
	<div class="admin_right">
     ...
     <table width="1193" border="0" cellpadding="0" cellspacing="0" class="listtable">
	  <tr>
		...
		<td width="108">微博</td>
		<td width="111">分类</td>
		<td width="165">微博名称</td>
		<td width="78">链接</td>
		<td width="193">粉丝数</td>
		<td width="150">直发报价</td>
		<td width="196">直发报价</td>
		<td width="52">修改</td>
		<td width="50">删除</td>
	  </tr>
	<?php 
	$numq=mysql_query("select * from tb_weibo");
	...
	$row=listquery("select * from tb_weibo order by id desc limit ".$num.",".$pageso."");
	foreach ($row as $v){
	?>
	  <tr>
		<td width="90"><input name="del_id[]" type="checkbox" id="del_id[]" value="<?=$v['id']?>" /></td>
		<td width="108"><?=$v[weibo]?></td>
		<td width="111"><?=$v[fenlei]?></td>
		<td width="165"><?=$v[weiboname]?></td>
		<td width="78"><?=$v[link]?></td>
		<td width="193"><?=$v[fensi]?></td>
		<td width="150"><?=$v[zhifa]?></td>
		<td width="196"><?=$v[zhuanfa]?></td>
		<td width="52"><a href="up_weibo.php?id=<?=$v[id]?>">修改</a></td>
...
$delid=$_GET['delid'];
if($delid){
mysql_query("delete from tb_weibo where id='$delid'");
 exit("<script>alert('删除成功!');window.location.href='list_weibo.php';</script>");
}
?>
...
$del_id=$_POST['del_id'];
     if($del_id!=""){ 
             ...
             for($i=0;$i<$del_num;$i++){ 
                 mysql_query("Delete from tb_weibo where id='$del_id[$i]'"); 				
             }  
            exit("<script>alert('删除成功!');window.location.href='list_weibo.php';</script>");
      }else{ 
            exit("<script>alert('请选择!');window.location.href='list_weibo.php';</script>");
      }
