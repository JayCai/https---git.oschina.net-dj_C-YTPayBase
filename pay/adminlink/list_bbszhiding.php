...
<div class="ureg">
  ...
  <!--right-->
	<div class="admin_right">
     ...
     <table width="1193" border="0" cellpadding="0" cellspacing="0" class="listtable">
	  <tr>
		...
		<td width="108">论坛</td>
		<td width="111">板块</td>
		<td width="165">板块地址</td>
		<td width="78">效果</td>
		<td width="193">置顶价格/天</td>
		<td width="150">加精价格/天</td>
		<td width="196">备注</td>
		<td width="52">修改</td>
		<td width="50">删除</td>
	  </tr>
	<?php 
	$numq=mysql_query("select * from tb_bbszhiding");
	...
	$row=listquery("select * from tb_bbszhiding order by id desc limit ".$num.",".$pageso."");
	foreach ($row as $v){
	?>
	  <tr>
		<td width="90"><input name="del_id[]" type="checkbox" id="del_id[]" value="<?=$v['id']?>" /></td>
		<td width="108"><?=$v[bbs]?></td>
		<td width="111"><?=$v[bk]?></td>
		<td width="165"><?=$v[bkadress]?></td>
		<td width="78"><?=$v[xiaoguo]?></td>
		<td width="193"><?=$v[zdprice]?></td>
		<td width="150"><?=$v[jjprice]?></td>
		<td width="196"><?=mb_substr($v[beizhu],0,12,'utf8')?></td>
		<td width="52"><a href="up_bbszhiding.php?id=<?=$v[id]?>">修改</a></td>
...
$delid=$_GET['delid'];
if($delid){
mysql_query("delete from tb_bbszhiding where id='$delid'");
 exit("<script>alert('删除成功!');window.location.href='list_bbszhiding.php';</script>");
}
?>
...
$del_id=$_POST['del_id'];
     if($del_id!=""){ 
             ...
             for($i=0;$i<$del_num;$i++){ 
                 mysql_query("Delete from tb_bbszhiding where id='$del_id[$i]'"); 				
             }  
            exit("<script>alert('删除成功!');window.location.href='list_bbszhiding.php';</script>");
      }else{ 
            exit("<script>alert('请选择!');window.location.href='list_bbszhiding.php';</script>");
      }
