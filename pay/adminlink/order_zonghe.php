  ...
  <form name="del_form" action="" method="post">
	<table width="1182" border="0" cellspacing="0" cellpadding="0" class="order2">
	  ...
	  <tr>
		...
		<td width="80"><a href="fabiao.php?id=<?=$v['id']?>&tb=order_zonghe" target="_blank">发布报表</a></td>
	  </tr>	
	  <?php
	  }
	  ?> 
...
if($ID){
    ...
    $f=query("select * from order_zonghe where id='$ID'");
	$zt=$f['type'];
	if($zt=='退单'){
	exit("<script>alert('不可操作!');window.location.href='order_zonghe.php?caozuoid=zonghe';</script>");
	}
	if($zt=='交易成功'){
	exit("<script>alert('不可操作!');window.location.href='order_zonghe.php?caozuoid=zonghe';</script>");
	}
	
    mysql_query("update order_zonghe set type='$type',liyou='$liyou' where id='$ID'");
		//退单操作
	$time=date("Y-m-d H:i:s");
	$order_id=$f['orderID'];//订单编号
	$order_user=$f['user'];//订单用户名	
	$order_cpname='综合套餐';//订单产品名称
    ...
    exit("<script>alert('操作成功!');window.location.href='order_zonghe.php?caozuoid=zonghe';</script>");
}
?>  
...
$del_id=$_POST['del_id'];
     if($del_id!=""){ 
             ...
             for($i=0;$i<$del_num;$i++){ 
                 mysql_query("Delete from order_zonghe where id='$del_id[$i]'"); 				
             }  
             exit("<script>alert('删除成功!');window.location.href='order_zonghe.php?caozuoid=zonghe';</script>");
      }else{ 
             exit("<script>alert('请选择!');window.location.href='order_zonghe.php?caozuoid=zonghe';</script>");
      }
