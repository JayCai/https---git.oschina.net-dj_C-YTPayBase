...
<div class="ureg">
  ...
  <!--right-->
	...
	<?php 
	$numq=mysql_query("select * from tb_zonghe");
	...
	$row=listquery("select * from tb_zonghe order by id desc limit ".$num.",".$pageso."");
	foreach ($row as $v){
	?>
	  <tr>
		<td width="90"><input name="del_id[]" type="checkbox" id="del_id[]" value="<?=$v['id']?>" /></td>
		<td width="108"><?=$v[leixing]?></td>	
		<td width="165" class="show"><?=mb_substr($v[taocanzy],0,10,'utf8')?><div class="showli"><?=$v[taocanzy]?></div></td>
		<td width="78"><?=$v[zhouqi]?></td>
		<td width="193"><?=$v[price]?></td>
		<td width="150"><?=$v[scprice]?></td>
		<td width="196"><?=mb_substr($v[beizhu],0,12,'utf8')?></td>
		<td width="52"><a href="up_zonghe.php?id=<?=$v[id]?>">修改</a></td>
...
$delid=$_GET['delid'];
if($delid){
mysql_query("delete from tb_zonghe where id='$delid'");
 exit("<script>alert('删除成功!');window.location.href='list_zonghe.php';</script>");
}
?>
...
$del_id=$_POST['del_id'];
     if($del_id!=""){ 
             ...
             for($i=0;$i<$del_num;$i++){ 
                 mysql_query("Delete from tb_zonghe where id='$del_id[$i]'"); 				
             }  
            exit("<script>alert('删除成功!');window.location.href='list_zonghe.php';</script>");
      }else{ 
            exit("<script>alert('请选择!');window.location.href='list_zonghe.php';</script>");
      }
