...
<div class="ureg">
  ...
  <!--right-->
	<div class="admin_right">
     ...
     <table width="1193" border="0" cellpadding="0" cellspacing="0" class="listtable">
	  <tr>
		<td width="90">选择</td>
		<td width="108">分类</td>
		<td width="111">微信名称</td>
		<td width="165">微信号</td>
		<td width="78">粉丝数</td>
		<td width="193">单图文报价</td>
		<td width="150">多图文报价</td>
		<td width="196">备注</td>
		<td width="52">修改</td>
		<td width="50">删除</td>
	  </tr>
	<?php 
	$numq=mysql_query("select * from tb_weixin");
	...
	$row=listquery("select * from tb_weixin order by id desc limit ".$num.",".$pageso."");
	foreach ($row as $v){
	?>
	  <tr>
		<td width="90"><input name="del_id[]" type="checkbox" id="del_id[]" value="<?=$v['id']?>" /></td>
		<td width="108"><?=$v[fenlei]?></td>
		<td width="111"><?=$v[weixinname]?></td>
		<td width="165"><?=$v[weixinhao]?></td>
		<td width="78"><?=$v[fensi]?></td>
		<td width="193"><?=$v[dantuwen]?></td>
		<td width="150"><?=$v[duotuwen]?></td>
		<td width="196"><?=mb_substr($v[beizhu],0,12,'utf8')?></td>
		<td width="52"><a href="up_weixin.php?id=<?=$v[id]?>">修改</a></td>
...
$delid=$_GET['delid'];
if($delid){
mysql_query("delete from tb_weixin where id='$delid'");
 exit("<script>alert('删除成功!');window.location.href='list_weixin.php';</script>");
}
?>
...
$del_id=$_POST['del_id'];
     if($del_id!=""){ 
             ...
             for($i=0;$i<$del_num;$i++){ 
                 mysql_query("Delete from tb_weixin where id='$del_id[$i]'"); 				
             }  
            exit("<script>alert('删除成功!');window.location.href='list_weixin.php';</script>");
      }else{ 
            exit("<script>alert('请选择!');window.location.href='list_weixin.php';</script>");
      }
