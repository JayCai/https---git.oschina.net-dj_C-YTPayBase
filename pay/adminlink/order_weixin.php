  ...
  <form name="del_form" action="" method="post">
	<table width="1182" border="0" cellspacing="0" cellpadding="0" class="order2">
	  ...
	  <tr>
		...
		<td width="100"><?=$v['user']?></td>		
		<td width="70"><?=$v['dantuwen']?></td>	
		<td width="70"><?=$v['duotuwen']?></td>		
		...
		<td width="109"><a href="show_weixin.php?id=<?=$v[id]?>&caozuoid=weixin">查看详情</a></td>
		<td width="80"><a href="fabiao.php?id=<?=$v['id']?>&tb=order_weixin" target="_blank">发布报表</a></td>
	  </tr>	
	  <?php
	  }
	  ?> 
...
if($ID){
    ...
    $f=query("select * from order_weixin where id='$ID'");
	$zt=$f['type'];
	if($zt=='退单'){
	exit("<script>alert('不可操作!');window.location.href='order_weixin.php?caozuoid=weixin';</script>");
	}
	if($zt=='交易成功'){
	exit("<script>alert('不可操作!');window.location.href='order_weixin.php?caozuoid=weixin';</script>");
	}
	
    mysql_query("update order_weixin set type='$type',liyou='$liyou' where id='$ID'");
	//退单操作
	$time=date("Y-m-d H:i:s");
	$order_id=$f['orderID'];//订单编号
	$order_user=$f['user'];//订单用户名	
	$order_cpname='微信资源';//订单产品名称
	$order_price1=$f['dantuwen'];//订单产品价格
	$order_price2=$f['duotuwen'];//订单产品价格
    ...
    exit("<script>alert('操作成功!');window.location.href='order_weixin.php?caozuoid=weixin';</script>");
}
?>  
...
$del_id=$_POST['del_id'];
     if($del_id!=""){ 
             ...
             for($i=0;$i<$del_num;$i++){ 
                 mysql_query("Delete from order_weixin where id='$del_id[$i]'"); 				
             }  
             exit("<script>alert('删除成功!');window.location.href='order_weixin.php?caozuoid=weixin';</script>");
      }else{ 
             exit("<script>alert('请选择!');window.location.href='order_weixin.php?caozuoid=weixin';</script>");
      }
