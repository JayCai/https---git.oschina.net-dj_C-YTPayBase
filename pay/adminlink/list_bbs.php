  ...
  <!--right-->
	<div class="admin_right">
     ...
     <table width="1193" border="0" cellpadding="0" cellspacing="0" class="listtable">
	  <tr>
		<td width="90">选择</td>
		<td width="108">论坛套餐</td>
		<td width="111">论坛类型</td>
		<td width="165">发布条数</td>
		<td width="78">周期</td>
		<td width="193">会员价</td>
...
<?php 
	$numq=mysql_query("select * from tb_bbs");
	...
	$row=listquery("select * from tb_bbs order by id desc limit ".$num.",".$pageso."");
	foreach ($row as $v){
	?>
	  <tr>
		<td width="90"><input name="del_id[]" type="checkbox" id="del_id[]" value="<?=$v['id']?>" /></td>
		<td width="108"><?=$v[bbstaocan]?></td>
		<td width="111"><?=$v[bbsleixing]?></td>
		<td width="165"><?=$v[fabutiao]?></td>
		<td width="78"><?=$v[zhouqi]?></td>
		<td width="193"><?=$v[price]?></td>
		<td width="150"><?=$v[shoulu]?></td>
		<td width="196"><?=mb_substr($v[beizhu],0,12,'utf8')?></td>
		<td width="52"><a href="up_bbs.php?id=<?=$v[id]?>">修改</a></td>
...
$delid=$_GET['delid'];
if($delid){
mysql_query("delete from tb_bbs where id='$delid'");
 exit("<script>alert('删除成功!');window.location.href='list_bbs.php';</script>");
}
?>
...
$del_id=$_POST['del_id'];
     if($del_id!=""){ 
             ...
             for($i=0;$i<$del_num;$i++){ 
                 mysql_query("Delete from tb_bbs where id='$del_id[$i]'"); 				
             }  
            exit("<script>alert('删除成功!');window.location.href='list_bbs.php';</script>");
      }else{ 
            exit("<script>alert('请选择!');window.location.href='list_bbs.php';</script>");
      }
