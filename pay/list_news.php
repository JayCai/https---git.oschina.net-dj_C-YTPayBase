   ...
   <body>   
	  ...
	  <?php
		 ...
		 <div class="bnav">
			...
			<a href="./list_dxwenzhang.php?caozuoid=pt3">代写文章</a>  
		 </div>
		 <div class="bnav">
		 ...
		 <form action="add_news.php" method="post">
			...
			<?php 
			   $sex=query("select count(id) from ordertop where user='$user' and tb='tb_news'");
			   $xge=$sex['count(id)'];
			   if($xge==0){
				  $xge=0;
			   }

			   $sexd=listquery("select * from ordertop  where user='$user' and tb='tb_news' order by id desc");
			   foreach ($sexd as $ve){
				  $vid=$ve[cpID];
				  $sexdd=query("select * from tb_news  where id='$vid'");
				  $showxuan.='<a>'.$sexdd['mtname'].'<a>'.'<a href="?del_id='.$ve[id].'"><font color=blue>点击删除</font></a>';
					 }

					 $del_id=$_GET['del_id'];
					 if($del_id){
						mysql_query("delete from ordertop where id='$del_id'");
						exit("<script>window.location.href='./list_news.php?caozuoid=pt1';</script>");	
					 }
				  ?>
			   ...
			   <?php 
				  $cpID=$_GET['mid'];//产品IP
				  if($cpID){
					 $zz=query("select * from ordertop where user='$user' and cpID='$cpID' and tb='tb_news'");
					 if(!$zz[id]){
						mysql_query("insert into ordertop (tb,cpID,user) values ('tb_news','$cpID','$user')");
						exit("<script>window.location.href='./list_news.php?caozuoid=pt1';</script>");
					 }else{
