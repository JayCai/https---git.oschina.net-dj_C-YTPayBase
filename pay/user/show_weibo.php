...
<div class="ureg">
  ...
  <!--right-->
	...
	$id=$_GET['id'];	
	$v=query("select * from order_weibo where id='$id'");
    ...
    <table width="1183" height="330" border="0" cellpadding="0" cellspacing="0" class="order3" >
	  ...
	  <tr>
		...
		<table width="858" height="39" border="0" cellpadding="0" cellspacing="0">
		  <tr>		   
			<td width="105">微博名称</td>
			<td width="157">微博链接</td>
			<td width="105">直发价格</td>
			<td width="105">转发发价格</td>
		...
		foreach ($bh as $e){
		if($e){	
		$x=query("select * from tb_weibo where id='$e'");
		?>
		<table width="858" height="35" border="0" cellpadding="0" cellspacing="0">
		   <tr>
			<td width="105"><?=$x['weiboname']?></td>
			<td width="105"><?=$x['link']?></td>
			<td width="105"><?=$x['zhifa']?></td>
			<td width="105"><?=$x['zhuanfa']?></td>
			<td width="105"><a href="up_weibo.php?id=<?=$e?>" target="_blank">查看详细</a></td>
		  </tr>
		</table>
