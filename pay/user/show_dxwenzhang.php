...
<div class="ureg">
  ...
  <!--right-->
	...
	$id=$_GET['id'];
	$v=query("select * from order_dxwenzhang where id='$id'");
	?>			
    <table width="1183" height="330" border="0" cellpadding="0" cellspacing="0" class="order3" >
	  ...
	  <tr>
		...
		foreach ($bh as $e){
		if($e){	
		$x=query("select * from tb_dxwenzhang where id='$e'");
		?>
		<table width="500" border="0" cellspacing="0" cellpadding="0">
		   <tr>			
			<td width="105"><?=$x['zishu']?></td>
			<td width="105"><?=$x['price']?></td>
			<td width="105"><a href="up_dxwenzhang.php?id=<?=$e?>" target="_blank">查看详细</a></td>
		  </tr>
		</table>
