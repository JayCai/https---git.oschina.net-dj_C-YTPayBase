...
<table class="table_weibo_list"  border="0" cellspacing="0" cellpadding="0" >
   <?php
       ...
       $numq=mysql_query("select id from tb_weibo  where weiboname like '%$so%' or link like '%$so%'");	   
	   require_once 'page.php';	  
	   $list=listquery("select * from tb_weibo where weiboname like '%$so%' or link like '%$so%' order by id desc limit ".$num.",20");
	   foreach ($list as $v){
	   ?> 
	     <tr>
			<td width="124" ><input name="numid[]" type="checkbox" value="<?=$v['id']?>" class="checkbox" style="margin-top:5px;" onclick="check();"/>选中发布</td>
			<td width="105"><?=$v['weibo']?></td>      
			<td width="143"><?=$v['weiboname']?></td> 
			<td width="102"><?=$v['fenlei']?></td> 
			<td width="251" style="text-align:center;"><a href="<?=$v['link']?>" target="_blank"><?=$v['link']?></a></td>	
			<td width="145"><?=$v['fensi']?></td>
