...
<html xmlns="http://www.w3.org/1999/xhtml">
   ...
   <body>
	  ...
	  <div class="addbox">
		 ...
		 <table width="1018" height="79"  border="0" cellpadding="0" cellspacing="0" style="margin-left:20px; margin-top:10px;">
			<tr>
			   <td width="243" height="45" style="color:blue; font-size:18px;">发布综合套餐文章</td>
			   <td width="642"></td>
			</tr>
			<tr>
			   ...
			   <td width="642">
				  <?php
					 $numid=listquery("select * from ordertop where user='$user' and tb='tb_zonghe'");//查询ordertop 产品ID系列表
					 foreach ($numid as $v){

						$a=query("select * from tb_zonghe where id='$v[cpID]'");
						$cpID.=$v[cpID].',';//产品ID
						$price[]=$a['price']; //产品价格
   ...
   <?php
	  ...
	  if($_POST['submitgo']){
		 ...
		 $wee=query("select * from order_zonghe where orderID='$orderID'");
		 if($wee['id']){
		 ...
		 $iR =mysql_query("insert into order_zonghe (orderID,user,cpID,price,content,liuyan,type,time,liyou,baobiao) 
		 values 
		 ...
		 if($tuijie){ 	
			...
			$iR =mysql_query("insert into ticheng_b (orderID,user,cpname,cpID,tb,price,tuijie,ticheng,time) values ('$orderID','$user','综合套餐','$cpID','tb_zonghe','$ture_price','$tuijie','$tcok','$time')");
			if( !$iR )
			{
			   die('Error: ' . mysql_error());
			}
		 ...
		 $iR =mysql_query("insert into xiaofei (orderID,user,cpname,cpID,tb,price,time) values ('$orderID','$user','综合套餐','$cpID','tb_zonghe','$ture_price','$time')"); 
		 if( !$iR )
		 {
			die('Error: ' . mysql_error());
		 }
   ...
   <?php 
	  ...
	  if($del_id){
		 ...
		 exit("<script>window.location.href='./add_zonghe.php';</script>");	
	  }
   ?>
   <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
	  ...
	  <table width="1018" height="656"  border="0" cellpadding="0" cellspacing="0" style="margin-left:20px; margin-top:10px;">	  
		 ...
		 <tr>
			...
			<td>
			   <input name="ture_price" value="<?=$ture_price?>" type="text" style="display:none;" /></td>
			<td width="133">
