...
<div class="ureg">
  ...
  <!--right-->
	...
	$id=$_GET['id'];	
	$v=query("select * from order_weixin where id='$id'");
	  ...
	  <tr>
		...
		<table width="858" height="39" border="0" cellpadding="0" cellspacing="0">
		  <tr>		   
			...
			<td width="105">微信号</td>
			<td width="105">单图文价格</td>
			<td width="105">多图文价格</td>
		...
		foreach ($bh as $e){
		if($e){	
		$x=query("select * from tb_weixin where id='$e'");
		?>
		<table width="858" height="35" border="0" cellpadding="0" cellspacing="0">
		   <tr>
			<td width="105"><?=$x['weixinname']?></td>
			<td width="105"><?=$x['weixinhao']?></td>
			<td width="105"><?=$x['dantuwen']?></td>
			<td width="105"><?=$x['duotuwen']?></td>
			<td width="105"><a href="up_weixin.php?id=<?=$e?>" target="_blank">查看详细</a></td>
		  </tr>
		</table>
